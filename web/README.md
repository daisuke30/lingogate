...
### 画面監査（ui-audit）— **デプロイ前に必ず実行する**

```bash
npm run build
npm run ui-audit                       # ローカルの本番ビルドを監査（preview は自動起動）
npm run ui-audit -- https://lingogate.pages.dev   # デプロイ後、本番に対しても再実行
```

主要5画面（ホーム／くわしくシート／設定／育成／ゲート）を実ブラウザで開き、
スクショを撮った上で**レイアウトをassertする**。320px幅の確認も含む。
1件でも失敗したら非ゼロで終了する＝これがデプロイの門番。

**なぜ必要か（LINGO-042）**: LINGO-040 のデプロイで、設定画面の全行が不可視という
重大な回帰を本番に出した。その時 Vitest は 432/432 green、`tsc -b` クリーン、build 成功、
本番URLは 200、ブラウザのコンソールエラーはゼロだった。**DOMは完全に正しく、
ただ描画されていなかった**（flexの圧縮で `overflow:hidden` の行コンテナが 2px に潰れていた）。
ユニットテストでは原理的に検出できない——レイアウト後のページの性質だから。
だから ui-audit は要素の存在ではなく**ジオメトリ**（「この行は40px以上あり、
画面内に収まり、クリップされていない」）をassertする。

スクショは人間が見るための証拠であって門番ではない。**門番はassertion**——
誰も開かないスクショは何も検出しない。ただし、デプロイ前には必ず自分の目でも
スクショを開いて確認すること（assertionは「崩れていない」ことは言えても
「意図した見た目か」は言えない）。
